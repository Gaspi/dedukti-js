...
function head_rewrite(state, red) {
  ...
  let reduct_step = execute_dtree(dtree.tree,truncated_stack, red);
  if (!reduct_step) { return null; }
  const [rhead, rstack] = to_state(reduct_step[0]);
  state[0] = rhead;
  state[1] = stack.slice(0,stack.length-dtree.arity).concat(rstack);
  return reduct_step[1];
}
...
function execute_dtree(dtree, stack, red) {
  ...
  if (dtree[c] == 'Switch') {
    ...
    return [meta_subst(dtree.rule.rhs, subst), dtree.rule.name]
  } else {
