...
function compute_dtree(m) {
  ...
  if (j<0) {
    return {[c]:'Test',
        match   : compute_matching_problem(m.rows[0], m.depths),
        fallback: compute_dtree({rows:m.rows.slice(1), depths:m.depths.slice(1)})
      }
  } else {
...
function get_db_index(p) {
  if (p[c]!='Var') { throw "DTree:\nIll-formed rule : only locally bound variables may be arguments of meta variables in LHS"; }
  return p.index;
}

function compute_matching_problem(row,depths) {
  const mvars = [];
  for (let i = 0; i < row.cols.length; i++) {
    const p = row.cols[i];
    if (p[c] == 'MVar') {
      mvars.push({ index:i, name:p.name, args:p.args.map(get_db_index)});
    }
  }
  return { match:mvars, rhs:row.rule.rhs, rule_name:row.rule.name };
}
